...
const SIDEBAR_COOKIE_MAX_AGE = TIME_CONSTANTS.WEEK;
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/70 ring-sidebar-ring flex h-10 shrink-0 items-center rounded-md px-2 text-xs font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-tertiary-foreground',
...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-tertiary-foreground',
...
const SIDEBAR_MENU_RANDOM_WIDTH_MIN = 50;
const SIDEBAR_MENU_RANDOM_WIDTH_MAX = 40;
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    return `${Math.floor(Math.random() * SIDEBAR_MENU_RANDOM_WIDTH_MAX) + SIDEBAR_MENU_RANDOM_WIDTH_MIN}%`;
  }, []);
