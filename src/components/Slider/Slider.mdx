...
function CustomMinMaxSlider() {
return <Slider defaultValue={[25]} min={10} max={50} step={1} />;
}
`}
/>
...
function CustomStepSlider() {
return <Slider defaultValue={[30]} max={100} step={5} />;
}
`}
/>
...
<Slider
      ...
      />
<div className="text-center text-sm">
Level: {value[0] <= 33 ? 'Low' : value[0] <= 66 ? 'Medium' : 'High'}
</div>
</div>
);
}
...
<Slider 
          ...
          max={100} 
          step={1}
          className="[&_[data-slot=slider-range]]:bg-stripes"
        />
</div>
</div>
);
...
- **Smooth Animations**: Provides smooth value transitions
