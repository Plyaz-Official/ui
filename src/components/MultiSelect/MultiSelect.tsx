...
export function MultiSelectTrigger({
  ...
  return (
    <PopoverTrigger asChild>
      ...
      >
        {children}
        <ChevronsUpDownIcon className='opacity-50 size-4 shrink-0' />
      </Button>
    </PopoverTrigger>
  );
}
      ...
      {[...selectedValues]
        ...
        .map(value => (
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <XIcon className='size-2 text-muted-foreground group-hover:text-destructive hover:cursor-pointer group-hover:cursor-pointer' />
            )}
          </Badge>
        ))}
      <Badge
